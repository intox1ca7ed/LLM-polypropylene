"""
Generate notebooks/02_baselines_and_residuals.ipynb leveraging baseline_utils.
"""
...
def main() -> None:
    notebook_path = Path("notebooks/02_baselines_and_residuals.ipynb")
    ...
    cells.append(
        nbf.v4.new_markdown_cell(
            ...
            "Quick baselines for polypropylene (PP) using naive/seasonal/ETS methods and "
            "crude-driven regressions. Outputs feed the next-stage feature work."
        )
    )
    ...
    cells.append(
        nbf.v4.new_markdown_cell("## Baseline forecasts (levels)")
    )
    ...
    cells.append(
        nbf.v4.new_markdown_cell(
            "_Figure note:_ The baseline overlays (naive and Holt-Winters where available) make it "
            "easy to see whether simple level extrapolations keep pace with the holdout reality."
        )
    )

    cells.append(
        nbf.v4.new_markdown_cell("## Crude-driven regression (if crude available)")
    )
    ...
    cells.append(
        nbf.v4.new_markdown_cell(
            "_Regression insight:_ The backtest above shows how a simple crude-lag OLS model "
            "tracks PP; compare the plotted curve and printed metrics/p-values to see which "
            "lags matter and whether the fit materially improves over naive baselines. In most "
            "samples the contemporaneous and 1-month crude shocks dominate—check the p-values "
            "and the reported R² to confirm the strength in your run."
        )
    )

    cells.append(
        nbf.v4.new_markdown_cell("## Idiosyncratic residual (PP minus crude-driven fit)")
    )
    ...
    cells.append(
        nbf.v4.new_markdown_cell(
            "_Residual context:_ The residual series isolates PP moves unexplained by crude—"
            "these are the innovations future news/NLP features should target."
        )
    )

    cells.append(
        nbf.v4.new_markdown_cell("## Regime visuals")
    )
    ...
    cells.append(
        nbf.v4.new_markdown_cell(
            "_Regime takeaway:_ STL separates long-term trend, recurring seasonality, and "
            "residual noise, while the rolling-vol chart compares PP and crude turbulence "
            "through time."
        )
    )

    cells.append(
        nbf.v4.new_markdown_cell("## Scorecard")
    )
    ...
    cells.append(
        nbf.v4.new_markdown_cell(
            "_Scorecard guide:_ Focus on RMSE/MAPE to compare models; the crude-driven row should "
            "show whether adding lagged crude returns yields material error reduction."
        )
    )
