"""
Generate the lean EDA notebook backed by src/eda_utils.
"""
...
NOTEBOOK_PATH = Path("notebooks/01_eda_pp_vs_oil.ipynb")
...
cells.append(
    nbf.v4.new_markdown_cell(
        ...
        "Explore whether PP prices co-move with propylene (PGP) and crude; establish "
        "sensible baselines before expanding into richer feature sets."
    )
)

cells.append(
    nbf.v4.new_markdown_cell(
        "Local files under `data/` are used throughout. Units, currencies, and "
        "frequencies may differ, so we normalize and resample to monthly for "
        "comparability."
    )
)

cells.append(
    nbf.v4.new_code_cell("%pip install -q pandas numpy matplotlib seaborn statsmodels")
)
...
cells.append(
    nbf.v4.new_markdown_cell(
        "## Discovery & loading\n\n"
        "Find candidate files by keyword, load each series with `eda_utils.load_series`, "
        "and capture metadata for traceability."
    )
)
...
cells.append(
    nbf.v4.new_markdown_cell(
        "_Table note:_ The discovery summary above lists every file that matched our keyword "
        "scan and highlights which date/value columns were inferred so you can quickly spot "
        "mislabelled sources."
    )
)
...
cells.append(
    nbf.v4.new_markdown_cell(
        "## Monthly alignment\n\n"
        "Resample each raw series to monthly means, keep one preferred series per class, "
        "and form wide outer/inner tables."
    )
)
...
cells.append(
    nbf.v4.new_markdown_cell(
        "_Audit note:_ Use the audit table to confirm coverage (first/last date, missing %), "
        "plus currency/unit hints pulled from the raw files."
    )
)
...
cells.append(
    nbf.v4.new_markdown_cell(
        "_Figure:_ Normalized level lines let us compare PP and crude trends on a common 100-base "
        "scale so different currencies/units do not mask co-movement."
    )
)
...
cells.append(
    nbf.v4.new_markdown_cell(
        "_Table detail:_ The return summary offers mean/volatility/skewness for each monthly "
        "log-return series so you can judge distribution shape before modelling."
    )
)
...
cells.append(
    nbf.v4.new_markdown_cell(
        "_Figures recap:_ Rolling β tracks relationship strength, the beta-scaled spread "
        "highlights sustained divergence, the lead/lag heatmap surfaces timing asymmetry, "
        "and the scatter pinpoints outlier months that may warrant narrative follow-up."
    )
)
